...
test('saved removed scenes are sanitized and library contains 44 available sounds', async ({ page }) => {
  ...
  await expect(library.locator('.scene-grid article')).toHaveCount(44);
  ...
  await expect(library.locator('.scene-grid article')).toHaveCount(30);
  await expect(library.locator('.scene-access-badge.is-free')).toHaveCount(12);
});
