...
- Every H5 App Store action must use Apple's region-neutral `https://apps.apple.com/app/id1461182261` path and navigate in the current window. Do not restore a fixed `/us/` storefront or `target="_blank"`. On iPhone WeChat, intercept the action and show the top-right default-browser guide with a copyable App Store link; Safari and other browsers must keep the direct current-window navigation.
