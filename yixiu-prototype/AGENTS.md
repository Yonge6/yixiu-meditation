...
- Every H5 App Store action must use Apple's region-neutral `https://apps.apple.com/app/id1461182261` path and navigate in the current window. Do not restore a fixed `/us/` storefront or `target="_blank"`. On iPhone WeChat, navigate the current window to the first-party `/download.html` handoff, preserve the validated attributed App Store target, and show the top-right default-browser guide with a copyable App Store link. Opening that handoff in the default browser must automatically continue to the App Store without a second download tap. Safari and other browsers must keep the direct current-window navigation.
...
- Match the H5 and native iPhone typography to 三慢问道: self-host `Noto Serif SC` for display, titles, and reflective copy, and `Noto Sans SC` for controls and supporting UI. The H5 bottom Sounds / Focus / Me navigation is intentionally web-native on both phone and desktop: use a conventional edge-to-edge bottom tab bar with three equal-width items, a top divider, and browser safe-area padding; do not copy the native app's floating rounded iOS capsule or its Home Indicator spacing. Keep the native SwiftUI app unchanged. The Me home/detail views continue to use the native iPhone app as their hierarchy source of truth, including compact top rhythm, back header, card proportions, labels, and active states. The H5 App Store download row must provide immediate pressed/status feedback as it opens the store.
...
- Keep the public H5 aligned with the accepted iOS visual language: a fresh unparameterized visit starts in English, while explicit `?lang=zh|en` links and stored manual choices still win; the Sounds header keeps the brand at left and orders App Store, Share, then the visible language control at right; FREE remains a text capsule while Plus uses the faceted gemstone mark; the accepted iOS artwork is also the source of truth for Still Water, Open Meadow, Oasis Rest, Ocean Passage, and Quiet Orbit.
