...
- Follow the 三慢问道 interaction structure for global language and More controls: the header uses one compact language action (`EN` on Chinese, `中文` on English), while More opens a full-height right drawer with a blurred backdrop, fixed bilingual brand/title header, and scrollable content. The drawer is the sole navigation hub for the sound player, full sound library, breathing practice, favorites/settings, philosophy, privacy, and support; do not restore the former bottom tab bar. Keep Yixiu's deep-water visual language rather than Wendao's paper palette.
...
- The right drawer uses the same two-level navigation contract as 三慢问道: its header is always visible, every secondary view has a Back action, Close remains available on every level, the backdrop and Escape close the drawer, focus stays inside while open, and returning to a view resets its scroll position.
