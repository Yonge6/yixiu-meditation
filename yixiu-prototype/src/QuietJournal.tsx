...
export function QuietJournal({ language, articleId, onArticle, onPractice }: {
  ...
  if (article) {
    ...
    return <article className="quiet-journal quiet-article">
      ...
      {article.sources.length > 0 && <section className="quiet-sources"><h4>{language === "zh" ? "资料与核验" : "Sources and verification"}</h4><ul>{article.sources.map(source => <li key={source.url}><a href={source.url}>{source[language]}</a></li>)}</ul></section>}
  ...
  return <section className="quiet-journal" aria-label={language === "zh" ? "一休日常文章" : "Quiet Journal articles"}>
    ...
    <div className="quiet-filters" aria-label={language === "zh" ? "文章分类" : "Article categories"}>
      {(["all", "sleep", "focus", "reset", "guide"] as const).map((key, index) => <button type="button" key={key} aria-pressed={category === key} onClick={() => setCategory(key)}>{(language === "zh" ? ["全部", "睡前", "专注", "片刻", "使用指南"] : ["All", "Rest", "Focus", "Pause", "Guides"])[index]}</button>)}
    </div>
    <div className="quiet-articles">{entries.filter(entry => category === "all" || entry.category === category).map(entry => <button className="quiet-card" type="button" key={entry.slug} data-analytics-event="yixiu_journal_read" data-analytics-value={entry.slug} onClick={() => onArticle(entry.slug)}>
      <img src={entry.image} alt="" width={entry.imageWidth} height={entry.imageHeight} loading="lazy" /><span className="quiet-card-copy"><span className="quiet-meta">{entry[language].category} · {entry[language].readTime}</span><strong>{entry[language].title}</strong><span>{entry[language].summary}</span><span className="quiet-card-link">{language === "zh" ? "读一读" : "Read the note"} ↗</span></span>
    </button>)}</div>
